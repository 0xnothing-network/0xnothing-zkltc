...
derived reserve target, the official zkLTC stablecoin, the approved NUSD
conversion/bridge route, and the major DEX. Graduation stays disabled unless the
route settles synchronously and returns the ERC-20 LP token expected by the
current router and locker; otherwise deploy only after the revised architecture
has passed audit.
