...
contract OracleNUSDTest is TestBase {
    ...
    function setUp() public {
        ...
        nusd = new OracleNUSD(oracle, address(this), type(uint256).max);

        vm.deal(USER, 20 ether);
        vm.deal(RESERVE_PROVIDER, 20 ether);
    }

    function testMetadataAndMainnetConfiguration() public view {
        ...
        assertEq(nusd.supplyCeilingNusd(), type(uint256).max, "unbounded supply ceiling");
    }

    function testMintsOneNusdPerDollarOfNativeCollateral() public {
        vm.prank(USER);
        ...
        assertEq(nusd.totalCollateralWei(), 1 ether, "reserve accounting");
        assertEq(address(nusd).balance, 1 ether, "native reserve");
        assertEq(nusd.reserveValueNusd(), 50 ether, "reserve value");
    }

    function testRedeemsAtCurrentOraclePrice() public {
        vm.prank(USER);
    ...
    function testSupplyCeilingStillWorksForExplicitlyCappedDeployments() public {
    ...
    function testPriceDropCanMakeReserveInsufficient() public {
        vm.prank(USER);
        nusd.mintAtOracle{value: 1 ether}(50 ether, USER);
