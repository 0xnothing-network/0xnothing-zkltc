...
function initializeProvider(): void {
  if (Object.getOwnPropertyDescriptor(window, 'zeroxnothing')) return;
  ...
  // Claim the namespace before announcing; the shared one only when unclaimed, so
  ...
  if (!(window as { ethereum?: unknown }).ethereum) {
    try {
      ...
      // Another wallet can own a non-configurable shared provider.
    }
  }
