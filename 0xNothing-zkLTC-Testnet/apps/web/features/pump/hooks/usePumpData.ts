...
export function usePumpPortfolio() {
  ...
  const held = markets.filter(
    (market) => (balancesQuery.data?.balances.get(market.tokenAddress.toLowerCase()) ?? 0n) > 0n,
  );
  ...
  return {
    ...
    created,
    held,
    balances: balancesQuery.data?.balances ?? new Map<string, bigint>(),
