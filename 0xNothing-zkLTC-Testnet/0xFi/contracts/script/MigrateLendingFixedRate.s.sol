...
interface VmMigrateLendingFixedRate {
...
contract MigrateLendingFixedRate {
    ...
    VmMigrateLendingFixedRate private constant vm =
        VmMigrateLendingFixedRate(address(uint160(uint256(keccak256("hevm cheat code")))));
    ...
    bytes32 private constant EXPECTED_IMPLEMENTATION_ID =
        keccak256("0xfi.lending.fixed-4.5-4-0.5.80-85-90.paused-bootstrap.v2");
    string private constant IMPLEMENTATION_STATUS = "fixed-rate-protocol-spread-80-85-90-paused-bootstrap-v2";
    ...
    function run() external returns (PooledNUSDLendingPool newLendingPool) {
        ...
        newLendingPool = new PooledNUSDLendingPool(NUSD, deployer, SUPPLY_CAP_NUSD, BORROW_CAP_NUSD, false);
        ...
        uint256 newShares = newLendingPool.bootstrapSupply(migratedNusd, deployer);
    ...
    function _validateUnfundedReplacement(PooledNUSDLendingPool pool, address deployer) private view {
        ...
        if (
            address(pool.nusd()) != NUSD || pool.owner() != deployer || pool.pendingOwner() != address(0)
                ...
                || !pool.collateralWithdrawalPaused() || pool.activated() || !pool.bootstrapOpen()
                || pool.totalSupply() != 0 || pool.totalBorrowed() != 0 || pool.totalBadDebtNusd() != 0
                || IERC20(NUSD).balanceOf(address(pool)) != 0 || pool.collateralAssetCount() != 3
        ) revert UnexpectedConfiguration();
    ...
    function _writePrediction(
        ...
        vm.serializeBool(key, "activationRequired", true);
        vm.serializeString(key, "implementationStatus", IMPLEMENTATION_STATUS);
        string memory json = vm.serializeString(key, "status", "lending-fixed-rate-migration-prediction");
        vm.writeJson(json, "./deployments/lending-fixed-rate.json");
    }
}
