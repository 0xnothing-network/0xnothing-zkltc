...
import type { ActivityPoint, DataEnvelope } from "@/lib/data";
import { queryGoldsky, unconfiguredEnvelope } from "@/lib/server/goldsky";
import { isFactoryPair, loadPairTail, pairForTokens, pairTokenMetadata } from "@/lib/server/rpcTail";
...
export async function GET(request: NextRequest) {
  ...
  if (!dynamicPool && !validPairs.includes(pair)) return NextResponse.json({ error: "Unsupported pair." }, { status: 400 });
  ...
  try {
    ...
    if (!pool) return NextResponse.json(unconfiguredEnvelope([], "Pair address is not configured for indexed activity."));
    ...
    let envelope: DataEnvelope<ActivityPoint[]>;
    try {
