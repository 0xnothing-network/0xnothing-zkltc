...
const IMPLEMENTATION_STATUS = "fixed-rate-protocol-spread-80-85-90-v1";
const IMPLEMENTATION_ID = keccak256(stringToHex("0xfi.lending.fixed-4.5-4-0.5.80-85-90.v1"));
...
const predictionPath = path.join(root, "contracts", "deployments", "lending-v2.json");
...
requireAddress(previous.lendingPool, oldPool, "top-level old lending pool");
requireAddress(previous.contracts?.lendingPool, oldPool, "nested old lending pool");
requireAddress(network.deployment?.contracts?.lendingPool, oldPool, "network old lending pool");
...
requireEqual(newSupplyPaused, false, "new pool supply pause");
requireEqual(newBorrowPaused, false, "new pool borrow pause");
requireEqual(newWithdrawalPaused, false, "new pool collateral-withdrawal pause");
...
const nextDeployment = {
  ...
  lendingV2Migration: migrationRecord,
  transactionHashes,
...
const finalizedPrediction = {
  ...prediction,
  broadcasted: true,
  status: "lending-v2-migration-finalized",
