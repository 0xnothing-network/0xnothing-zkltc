...
import { lendingImplementationState } from "./lib/lending-implementation.mjs";
...
const collateralDefinitions = [
  ["wzkLTC", addresses.wzkLTC, addresses.ltcOracle, deployment.wzkLtcCollateralCap],
  ["nBTC", addresses.nBTC, addresses.btcOracle, deployment.nBTCLendingCollateralCap || deployment.nBTCDebtCeiling],
  ["nETH", addresses.nETH, addresses.ethOracle, deployment.nETHLendingCollateralCap || deployment.nETHDebtCeiling],
];
const collateral = [];
for (const [symbol, asset, oracle, cap] of collateralDefinitions) {
  const [config, feed, fresh] = await Promise.all([
    publicClient.readContract({ address: addresses.lending, abi: lendingAbi, functionName: "collateralConfigs", args: [asset] }),
  ...
  const expectedFeed = addressOf(network.dia.feeds[symbol], `${symbol} DIA feed`);
  const configured = config[0].toLowerCase() === oracle.toLowerCase()
    && config[1] === BigInt(cap)
    && config[2] === 5000
    && config[3] === 6500
    && config[4] === 500
...
for (const [symbol, asset, expectedPair, expectedGauge, gaugeFactory] of expectedPairs) {
  const [pair, gauge, lpSupply] = await Promise.all([
...
const reserveMigration = deployment.synthSafetyReserveMigration;
if (reserveMigration) {
...
const localManifestConsistent = Object.entries(localBindings).every(([addressKey, manifestKey]) => (
  ...
  && network.deployment?.lendingImplementationStatus === lendingImplementation.status
  && Boolean(network.deployment?.lendingImplementationMigrationRequired)
    === lendingImplementation.migrationRequired;
...
const operational = topologyOk
  && graduationReady
  && oraclesFresh
  && lendingImplementation.badDebtInclusiveBorrowCapFix;
